...
| P0 | GET/egress | `/v6/blob_io` | `hits`; `flow_out`/`cdn_flow_out` in bytes; fixed `g=5min` for the latest rate and `g=day` for current-month direct-egress usage |
| P0 | Customer workload PUT count | `/v6/rs_put?select=hits` | Read-only query for PUT requests generated by the customer's workload; the exporter does not perform object PUTs; fixed `g=5min` for the latest rate and `g=day` for current-month usage |

For the current capacity/object APIs, `begin` is inclusive and `end` is exclusive, both formatted as `YYYYMMDDHHmmss`. Historical data usually supports only day granularity; the current day supports 5min/hour/day. `blob_io` and `rs_put` support 5min/hour/day/month. Current-month usage deliberately sums bounded `g=day` points from the Asia/Shanghai month boundary through the safe current time instead of relying on undocumented partial-`month` bucket behavior. Legacy statistics, cross-region tasks, and early-deletion analysis are outside the MVP.
...
| `qiniu_kodo_usage_egress_bytes` | Gauge | `bucket,region,route,period` | Direct egress accumulated from the current local month boundary through the safe current time; currently `route="direct"`, `period="current_month"` |
| `qiniu_kodo_usage_requests` | Gauge | `bucket,region,operation,period` | PUT requests accumulated from the current local month boundary through the safe current time; currently `operation="put"`, `period="current_month"` |

`blob_io` and `rs_put` return increments over historical intervals, not monotonically increasing values over the exporter process lifetime. Therefore, their metrics must not use a `_total` suffix or masquerade as Counters. Rate metrics request `g=5min`, divide by 300, validate point alignment, and verify that adjacent points are exactly 5 minutes apart. Current-month usage metrics remain Gauges because they reset at the month boundary and Qiniu may revise metering data. Do not export a rate when a bucket is missing or only one point is available and its window cannot be confirmed. If other granularities are supported later, convert using the defined duration of the requested granularity; do not infer coverage from the distance between points across missing buckets.
...
| Kodo GET/PUT/egress and current-month summary | 30 minutes (`collection.intervals.kodo_activity`) | Read the latest safe five-minute rate bucket and sum upstream day buckets from the local month boundary through the safe current time for direct egress and PUT usage |
