...
semantic color blocks. Bucket Inventory also joins the latest storage/object
gauges and the natural-month direct-egress/PUT gauges for each bucket. CDN
usage cards show the last complete hour, today, and current month without
trying to reconstruct totals from sampled Prometheus Gauge history. Monthly
Top 5 bandwidth follows the same domains selected by monthly traffic. Billing
snapshot values remain separate KPI cards.
